...
#else
		fprintf(stderr,"**************************************\n");
		fprintf(stderr,"Please wait for 5 seconds.\n");
		fprintf(stderr,"Transmission algorithm modified ");
		fprintf(stderr,"by Takayuki NITTA at Oct. 8, 2011.\n");
		fprintf(stderr,"**************************************\n");
#define WINDOW 4
		for(i=0;i<WINDOW;i++)
		  putbyte(enable[i]);
		for(i = WINDOW;i < BIN3069_SIZE;i++) {
			putbyte(enable[i]);
			getbyte();
		}
		for(i=0;i<WINDOW;i++)
